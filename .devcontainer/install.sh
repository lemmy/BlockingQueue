...
## Update ubuntu's package list (in the background)
## Captures the PID of the background apt-get update process.
...
WS=$(dirname $(dirname $(realpath "$0")))

## Install TLA+ Tools
wget -qN https://nightly.tlapl.us/dist/tla2tools.jar -P tools/
echo "alias tlcrepl='java -cp $WS/tools/tla2tools.jar tlc2.REPL'" >> $HOME/.bashrc
echo "alias tlc='java -cp $WS/tools/tla2tools.jar tlc2.TLC'" >> $HOME/.bashrc
...
wget -qN https://github.com/tlaplus/tlapm/releases/download/1.6.0-pre/tlapm-1.6.0-pre-x86_64-linux-gnu.tar.gz -P /tmp
tar -xzf /tmp/tlapm-1.6.0-pre-x86_64-linux-gnu.tar.gz -C tools/
echo 'export PATH=$PATH:$WS/tools/tlapm/bin' >> $HOME/.bashrc

## Install Apalache
wget -qN https://github.com/apalache-mc/apalache/releases/latest/download/apalache.tgz -P /tmp
mkdir -p tools/apalache
tar xvfz /tmp/apalache.tgz --directory tools/apalache/
echo 'export PATH=$PATH:$WS/tools/apalache/bin' >> $HOME/.bashrc
tools/apalache/bin/apalache-mc config --enable-stats=true

## Synchronize (wait) for the apt-get update to finish
wait $PID
